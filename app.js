(() => {
  ...
  function loadInventory() {
    try {
      ...
      if (!raw) return clone(starterInventory);
      const parsed = JSON.parse(raw);
      if (!parsed || !Array.isArray(parsed.spools)) throw new Error('Invalid inventory backup');
      return parsed.spools.map(normalizeSpool);
    } catch (error) {
      console.warn('Could not load inventory; using starter data.', error);
      return clone(starterInventory);
    }
  }

  function normalizeSpool(spool) {
    return {
      id: String(spool.id || '').trim(), brand: String(spool.brand || 'Unknown').trim() || 'Unknown',
      material: String(spool.material || 'Unknown').trim() || 'Unknown', colorName: String(spool.colorName || 'Unknown').trim() || 'Unknown',
      colorHex: /^#[0-9a-f]{6}$/i.test(spool.colorHex || '') ? spool.colorHex : '#64748b',
      spoolType: String(spool.spoolType || 'Unknown'), startWeight: validNum(spool.startWeight) && Number(spool.startWeight) > 0 ? Number(spool.startWeight) : 1000,
      visualPercent: validNum(spool.visualPercent) ? Math.min(100, Math.max(0, Number(spool.visualPercent))) : null,
      gross: validNum(spool.gross) ? Math.max(0, Number(spool.gross)) : null, tare: validNum(spool.tare) ? Math.max(0, Number(spool.tare)) : null,
      location: String(spool.location || '').trim(), confidence: String(spool.confidence || 'Unknown'), notes: String(spool.notes || '').trim(),
      updatedAt: spool.updatedAt || null
    };
  }

  function saveInventory() {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({version:APP_VERSION, savedAt:today(), spools:inventory}));
  }
  ...
  function renderDashboard() {
    ...
    const low = inventory.filter(s => ['Low','Very low'].includes(statusFor(measurement(s).percent))).length;
    const unknown = inventory.filter(s => statusFor(measurement(s).percent) === 'Unknown').length;
    $('metrics').innerHTML = [
      ['Active spools', inventory.length, 'Unique records in this browser'],
      ['Known filament', `${(knownGrams/1000).toFixed(1)} kg`, `${known.length} spools have a usable estimate`],
      ['Measured', measured, 'Spools with gross + tare weight'],
      ['Needs attention', low + unknown, `${low} low • ${unknown} unknown fill`]
    ...
    $('materialGrid').innerHTML = Object.entries(materials).sort((a,b) => b[1]-a[1] || a[0].localeCompare(b[0])).map(([name,count]) => `<article class="material-card"><div class="row"><strong>${esc(name)}</strong><strong>${count}</strong></div><small>${Math.round(count/inventory.length*100)}% of active spools</small></article>`).join('');

    const urgent = inventory.map(s => ({s,m:measurement(s)})).filter(x => x.m.percent !== null).sort((a,b) => a.m.percent-b.m.percent).slice(0,4);
    const unknownItems = inventory.filter(s => measurement(s).percent === null).slice(0, Math.max(0, 4-urgent.length)).map(s => ({s,m:measurement(s)}));
    const queue = [...urgent, ...unknownItems];
    $('priorityList').innerHTML = queue.length ? queue.map(({s,m}) => {
      const status = statusFor(m.percent);
      return `<div class="quick-item"><i class="dot" style="color:${STATUS_COLORS[status]};background:${STATUS_COLORS[status]}"></i><div><strong>${esc(s.id)} · ${esc(s.brand)} ${esc(s.material)}</strong><br><span>${esc(s.colorName)}</span></div><span>${m.percent === null ? 'Unknown' : `${Math.round(m.percent)}%`}</span></div>`;
    }).join('') : '<div class="empty"><strong>No urgent items</strong>Everything has a known healthy level.</div>';
  }
  ...
  function filteredInventory() {
    ...
    return inventory.filter(s => {
      const hay = [s.id,s.brand,s.material,s.colorName,s.spoolType,s.location,s.notes].join(' ').toLowerCase();
      return (!q || hay.includes(q)) && (!material || s.material === material) && (!status || statusFor(measurement(s).percent) === status) && (!location || s.location === location);
  ...
  function renderInventory() {
    ...
    $('inventoryGrid').innerHTML = list.map(s => {
      const m = measurement(s), status = statusFor(m.percent), color = STATUS_COLORS[status], pct = m.percent === null ? 0 : Math.max(0,Math.min(100,m.percent));
      return `<article class="spool-card" data-id="${esc(s.id)}"><div class="spool-head"><div class="swatch" style="background:${esc(s.colorHex)}"></div><div class="spool-title"><h4>${esc(s.id)} · ${esc(s.colorName)}</h4><p>${esc(s.brand)} · ${esc(s.material)}</p></div><span class="confidence">${esc(s.confidence)}</span></div><div class="spool-body"><div class="fill-top"><strong>${m.percent === null ? '—' : `${Math.round(m.percent)}%`}</strong><small>${m.grams === null ? 'Fill unknown' : `~${Math.round(m.grams)} g`}<br>${esc(m.source)}</small></div><div class="progress"><i style="width:${pct}%;background:${color}"></i></div><div class="meta"><div><span>Status</span><strong style="color:${color}">${status}</strong></div><div><span>Location</span><strong>${esc(s.location || 'Unassigned')}</strong></div><div><span>Spool</span><strong>${esc(s.spoolType)}</strong></div><div><span>Updated</span><strong>${s.updatedAt ? new Date(s.updatedAt).toLocaleDateString() : 'Photo audit'}</strong></div></div></div><div class="card-actions"><button class="btn" data-action="weigh" data-id="${esc(s.id)}" type="button">Weigh</button><button class="btn" data-action="edit" data-id="${esc(s.id)}" type="button">Edit</button><button class="btn btn-danger" data-action="delete" data-id="${esc(s.id)}" type="button">Delete</button></div></article>`;
    }).join('');
  }
  ...
  function saveSpoolFromForm(event) {
    ...
    const gross = num($('grossEdit').value), tare = num($('tareEdit').value);
    ...
    const spool = normalizeSpool({
      id, brand:$('brand').value, material:$('material').value, colorName:$('colorName').value, colorHex:$('colorHex').value,
      spoolType:$('spoolType').value, startWeight:num($('startWeight').value) || 1000, visualPercent:num($('visualPercent').value),
      gross, tare, location:$('location').value, confidence:$('confidence').value, notes:$('notes').value, updatedAt:today()
    });
    if (originalId) inventory = inventory.map(s => s.id === originalId ? spool : s); else inventory.push(spool);
    saveInventory(); closeSpoolDialog(); renderAll(); showToast(originalId ? 'Spool updated.' : 'Spool added.');
  }
  ...
  function updateCalcPreview() {
    ...
    const spool = inventory.find(s => s.id === id);
    const gross = num($('grossWeight').value), tare = num($('tareWeight').value);
    $('calcGross').textContent = gross === null ? '—' : `${gross} g`;
    $('calcTare').textContent = tare === null ? '—' : `${tare} g`;
    if (!spool || gross === null || tare === null || gross < tare) {
      $('calcRemaining').textContent = '—'; $('calcPercent').textContent = '—'; $('calcStatus').textContent = gross !== null && tare !== null && gross < tare ? 'Check weights' : '—'; return;
    }
    const grams = Math.max(0, gross - tare), pct = Math.max(0, Math.min(100, grams / (spool.startWeight || 1000) * 100));
    $('calcRemaining').textContent = `${Math.round(grams)} g`;
    $('calcPercent').textContent = `${pct.toFixed(1)}%`;
    $('calcStatus').textContent = statusFor(pct);
  }

  function saveMeasurement(event) {
    event.preventDefault();
    const id = $('weighSpool').value, spool = inventory.find(s => s.id === id);
    const gross = num($('grossWeight').value), tare = num($('tareWeight').value);
    ...
    spool.gross = gross; spool.tare = tare; spool.updatedAt = today();
    const location = $('weighLocation').value.trim(); if (location) spool.location = location;
    const note = $('weighNotes').value.trim(); if (note) spool.notes = [spool.notes, `${new Date().toLocaleDateString()}: ${note}`].filter(Boolean).join(' • ');
    saveInventory(); renderAll(); updateCalcPreview(); showToast(`${id} measurement saved.`);
  }
  ...
  function exportJson() {
    download(`filament-inventory-${new Date().toISOString().slice(0,10)}.json`, JSON.stringify({version:APP_VERSION, exportedAt:today(), spools:inventory}, null, 2), 'application/json');
  }

  function csvCell(value) { const s = String(value ?? ''); return /[",\n]/.test(s) ? `"${s.replace(/"/g,'""')}"` : s; }
  function exportCsv() {
    const headers = ['ID','Brand','Material','Color','Spool Type','Starting g','Visual %','Gross g','Tare g','Effective Remaining g','Effective %','Status','Location','Confidence','Notes','Updated'];
    const rows = inventory.map(s => { const m = measurement(s); return [s.id,s.brand,s.material,s.colorName,s.spoolType,s.startWeight,s.visualPercent ?? '',s.gross ?? '',s.tare ?? '',m.grams ?? '',m.percent ?? '',statusFor(m.percent),s.location,s.confidence,s.notes,s.updatedAt ?? '']; });
  ...
  async function importJson(file) {
    try {
      ...
      inventory = incoming; saveInventory(); renderAll(); showToast('Backup imported.');
  ...
  function bindEvents() {
    ...
    $('dialogCloseBtn').addEventListener('click', closeSpoolDialog); $('cancelSpoolBtn').addEventListener('click', closeSpoolDialog);
    ...
    $('clearFiltersBtn').addEventListener('click', () => { $('searchInput').value=''; $('materialFilter').value=''; $('statusFilter').value=''; $('locationFilter').value=''; renderInventory(); });
    $('inventoryGrid').addEventListener('click', event => {
      ...
      if (btn.dataset.action === 'delete' && confirm(`Delete ${spool.id} — ${spool.colorName}?`)) { inventory = inventory.filter(s => s.id !== spool.id); saveInventory(); renderAll(); showToast('Spool deleted.'); }
    });
    ...
    $('resetBtn').addEventListener('click', () => { if (!confirm('Reset all local edits and restore the 21-spool photo-audited starter inventory?')) return; inventory = clone(starterInventory); saveInventory(); renderAll(); showToast('Starter inventory restored.'); });
  }

  function init() {
    inventory = inventory.map(normalizeSpool);
    bindEvents(); renderAll(); updateCalcPreview();
    if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost')) navigator.serviceWorker.register('/sw.js').catch(error => console.warn('Service worker registration failed', error));
  }
